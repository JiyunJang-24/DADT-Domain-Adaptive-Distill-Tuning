...
def create_infos(input_folder, output_folder):
    folders = os.listdir(input_folder)
    pool = Pool(processes = 18)
    
    process_single = partial(
        process_single_sequence, input_folder = input_folder, output_folder = output_folder
    )
    infos = list(tqdm(pool.map(process_single, folders), total = len(folders)))
...
if __name__ == '__main__':
    ...
    create_infos(input_folder, output_folder)
