...
def cal_bev_attention_loss2_no_roi_add(batch_teacher, batch, index = None, mode = None):
    ...
    for i in range(batch_size):
        ...
        for j in range(roi_size):
            ...
            # Skip instances with class label 0
            if class_label != 1 and class_label != 2 and class_label != 3 and class_label != 4 and class_label != 5 and class_label != 6 and class_label != 7 and class_label != 8 and class_label != 9 and class_label != 10:
                continue
...
def train_model_mimic(model, model_teacher, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if supervised is not None and EMA != 0 and cur_epoch != total_epochs:
                if cur_epoch % 5 ==0:
                    ...
                    for key in teacher_params.keys():
                        model_params[key] = alpha * model_params[key] + (1 - alpha) * teacher_params[key]
