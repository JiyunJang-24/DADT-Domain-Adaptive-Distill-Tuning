    ...
    for b_idx in range(batch_size):
        ...
        if 'pred_boxes' in pred_dicts[b_idx]:
            ...
            if cfg.SELF_TRAIN.get('NEG_THRESH', None):
                ...
                #import pdb; pdb.set_trace()
                labels_remove_scores = np.array([cfg.SELF_TRAIN.NEG_THRESH[label - 1] for label in pred_labels])
                #import pdb; pdb.set_trace()
            ...
            labels_ignore_scores = np.array([cfg.SELF_TRAIN.SCORE_THRESH[label - 1] for label in pred_labels])
