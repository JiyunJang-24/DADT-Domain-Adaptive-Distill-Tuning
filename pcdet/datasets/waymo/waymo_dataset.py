...
class WaymoDataset(DatasetTemplate):
    ...
    """
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, use_ori=False, teacher_tag=None, logger=None, is_distill = False):
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger, use_ori=use_ori, is_distill = is_distill
        )
        self.use_ori = use_ori
        self.teacher_tag = teacher_tag
        if 'infos_seed' in dataset_cfg:
            self.infos_seed = dataset_cfg['infos_seed']
        else:
            self.infos_seed = 0
        if self.oss_path is not None:
            ...
            #self.use_ori = False
            if self.use_ori:
                self.data_path = self.root_path / self.dataset_cfg.PROCESSED_DATA_TAG / self.teacher_tag
                
            elif self.use_ori is False and self.dataset_cfg.get('LIDAR_DISTILLATION', None): #and self.training:
                self.data_path = self.root_path / self.dataset_cfg.PROCESSED_DATA_TAG / str(self.dataset_cfg['LIDAR_DISTILLATION'][0])
            else:
                self.data_path = self.root_path / self.dataset_cfg.PROCESSED_DATA_TAG / '64'
    ...
    def set_split(self, split):
        ...
        self.split = split
        if 'DATA_SIZE' in self.dataset_cfg:
            split_dir = os.path.join(self.root_path, 'ImageSets', self.dataset_cfg['DATA_SIZE'], self.split+'.txt')
        else:
    ...
    def include_waymo_data(self, mode):
        ...
        for k in tqdm(range(len(self.sample_sequence_list))):
            ...
            if self.oss_path is None:
                ...
                info_path = self.root_path / self.dataset_cfg.PROCESSED_DATA_TAG / 'default' / sequence_name / ('%s.pkl' % sequence_name)
                #info_path = self.data_path / sequence_name / ('%s.pkl' % sequence_name)
